...
function iterate() {
  ...
  for(var i=0;i<pop.length;i++) {
    ...
    ctx.fillText(i, net.xPos, net.yPos);
    
    ctx.fillText("U", net.eyeUpX, net.eyeUpY);
    ctx.fillText("D", net.eyeDownX, net.eyeDownY);
    ctx.fillText("L", net.eyeLeftX, net.eyeLeftY);
    ctx.fillText("R", net.eyeRightX, net.eyeRightY);
  }
...
function getSpeedOut(net) {
  net.speed = (net.upIn * net.up.speedW + net.downIn * net.down.speedW + net.leftIn * net.left.speedW + net.rightIn * net.right.speedW + net.h1Val * net.h1.speedW
  + net.h2Val * net.h2.speedW + net.h3Val * net.h3.speedW + net.h4Val * net.h4.speedW)/8;
}
...
//center x= cX
  ...
  // PI
  net.eyeDownX = cX - 30*Math.cos(net.rot + Math.PI);
  net.eyeDownY = cY - 30*Math.sin(net.rot + Math.PI);
  // PI/2
...
function getProx(net,sPosX, sPosY) {
  ...
  for(i=0;i<pop.length;i++){
    distance = findDis(sPosX,sPosY, pop[i].xPos, pop[i].yPos);
...
//    move:0, // is it moving or not? may change to boolean.
//    speed:0, //value between -max speed and max speed (can back up).
